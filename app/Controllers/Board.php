...
class Board extends BaseController
{
    ...
    public function list()
    {
        ...
        /***    /검색 기능 끝   ***/
        $option["perPage"] = 10;
        $option['orderBy'] = "bod_group desc,bod_sort desc";
    ...
    public function write($reply = '')
    {
        ...
        if ($idx) {
            ...
            if (!($origin['bod_mem_id'] === $this->mem_info['mem_id'] || $this->mem_info['mem_level'] === '90')) {
        ...
        if ($this->request->getMethod() === 'post') { //-- 저장
    ...
    public function delete()
    {
        ...
        $origin = $this->model->find($idx);
        if (!($origin['bod_mem_id'] === $this->mem_info['mem_id'] || $this->mem_info['mem_level'] === '90')) {
            alert("권한이 없습니다.", $this->cont_url . "/" . $this->boc_code);
        }


        $is_reply = $this->model->where(['bod_group' => $origin['bod_group'], 'bod_level >' => $origin['bod_level']])->countAllResults();
        if ($is_reply) {
            alert("답글이 있으면 삭제할 수 없습니다.", $this->cont_url . "/" . $this->boc_code);
        }

        $ready_to_del = array();
        $bof_arr = $this->BoardFileModel->where('bof_bod_idx', $idx)->get()->getResultArray();
        foreach ($bof_arr as $bof) {
            $this->BoardFileModel->del($bof['bof_idx']);
            $ready_to_del[] = $bof["bof_file_save"];
        }
