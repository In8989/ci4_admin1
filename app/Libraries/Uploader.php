...
class Uploader
{
    ...
    public function upload($path)
    {
        ...
        foreach ($files as $file) {
            ...
            if ($file->getError() != 0) {   //  0 업로드가 성공, 그 외는 실패 (https://www.php.net/manual/en/features.file-upload.errors.php)
                ...
                $fileInfo[$key]['hasError'] = false;  //  오류 플래그
                if ($file->hasMoved() === false) {  // 파일이 임시 디렉토리에서 실제 저장 디렉토리로 이동했는지 알아보려면 ->hasMoved()를 사용합니다.

                    if (array_search($file->guessExtension(), $this->allow_type) === false) { //  허용한 타입의 확장자인지 확인
    ...
    public function file_del($ready_to_del)
    {
        foreach ($ready_to_del as $path)
