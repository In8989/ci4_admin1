    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	...
    	// 부모창에서 저장 경로 획득
    	var fPath = opener.parent.fnSmartEditorFilePathGet();
	...
	function onAjaxError (){
     ...
     function uploadImage (e){
    	 if(!bSupportDragAndDropAPI || nMode==0){
    		 generalUpload();
    	 }else{
    		 html5Upload();
    	 }
 	...
 	function callFileUploader (){
		...
		var fPath = opener.parent.fnSmartEditorFilePathGet();
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php?fPath='+fPath,	//샘플 URL입니다.
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	function toggle_upload_mode(){
		...
		if(bSupportDragAndDropAPI && nMode==0){	// 드래그&드랍 파일등록
			...
			nMode = 1;
		} else if(!bSupportDragAndDropAPI || nMode==1) {	// 일반 파일 선택형
